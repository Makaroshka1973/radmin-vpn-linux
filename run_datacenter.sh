...
# A packager (Nix store, /opt, container image) can pre-set NOVNC_PATH to skip
# the distro guesses below; it is still validated like any other candidate.
for p in \
    "${NOVNC_PATH:-/nonexistent}" \
    /usr/share/novnc \
    /usr/share/novnc/www \
